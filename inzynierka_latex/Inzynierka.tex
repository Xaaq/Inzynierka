...
\begin{document}
	...
	\textit{W tym rozdziale opisany będzie proces zbierania danych za pomocą symulatora PSP, tworzenie modelu uczenia maszynowego oraz optymalizacji jego wartości wyjściowych.}
		...
		Po przeprowadzeniu eksperymentów ze wszystkimi powyższymi kombinacjami parametrów wysrysowałem wykresy krzywych uczących dla najlepszych przypadków na rysunkach \ref{fig: drugi_model_przy_regularyzacji_l1_wykresy_uczenia} i \ref{fig: drugi_model_przy_regularyzacji_l2_wykresy_uczenia} oraz wykresy krzywych uczących dla zmiennej ilości danych na rysunkach \ref{fig: drugi_model_przy_regularyzacji_l1_learning_curves} i \ref{fig: drugi_model_przy_regularyzacji_l2_learning_curves}. Dodatkowo zestawienie średnich \textit{MSE} dla owych kombinacji umiesciłem w tabeli \ref{srednie_wartosci_mse_przy_regularyzacji}.
		...
		\begin{table}[t]
			...
			\end{mytabular}
			\label{srednie_wartosci_mse_po_odrzuceniu_czesci_danych}
		\end{table}
		...
		\item Analizując tabelę \ref{srednie_wartosci_mse_po_odrzuceniu_czesci_danych} wygląda na to, że tak jak poprzednio - funkcja regularyzacji \textit{L1} z parametrem $\lambda$ równym \textbf{1} daje najlepsze rezultaty. Jeznakże nie wiele gorszym zestawem parametrów, ze zbliżonym \textit{MSE} danych testowych odznacza się ta sama funkcja z parametrem $\lambda$ równym \textbf{0,3}.
		\item Patrząc na wykresy krzywych uczących dla najlepszych przypadków widać, że niezależnie czy użyta została funkcja regularyzacji \textit{L1} czy \textit{L2}, przy parametrze $\lambda$ równym \textbf{1} sieć przyucza się minimalnie lepiej do danych treningowych. Pamiętając co było źródłe dotychczasowego problemu można wywnioskować, że takie zachowanie może być spowodowane przez szumy zbioru danych, a sposobem ich wyeliminowania było by zebranie danych z większej ilości symulacji przeprowadzonych z wyższym parametrem \textit{Repeat number}. Mając na uwadze powyższą informację wydaje się, że parametr $\lambda$ równy \textbf{0,3} będzie najlepszym wyborem do dalszej nauki przy obecnych danych.
		\end{itemize}
		
		Podsumowując, wynikiem badań w tym rozdziale jest znaleziona najlepsza kombinacja parametrów regularyzacji dla obecnych danych - funkcja \textit{L1} z parametrem $\lambda$ równym \textbf{0,3}. Jednakże ze względu na podejrzenie, że przez niską ilość powtórzeń pojedynczej symulacji, w zbiorze danych wyjściowych istnieją szumy, których eliminacja podniosłaby efektowność sieci przy parametrze $\lambda$ równym \textbf{1}, postanowiłem ponownie zebrać większą ilość danych symulacyjnych.
